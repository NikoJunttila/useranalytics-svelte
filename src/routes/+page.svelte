...
<div id="blur" on:mousemove={handleMousemove} class="z-10 overflow-hidden">
  <div class="w-full bg min-h-screen flex flex-col items-center justify-center p-5">
    ...
    >
      ...
      >
        ...
        <a class="mt-4" href="/example"><span>{$t("homepage.how")}</span></a>
      </div>
    </main>
  ...
  <div
    id="info"
    class="flex flex-col items-center mt-5 mx-2 gap-5 lg:gap-0 scroll-m-12 sm:text-2xl
...
<style>
  .bg {
    background-image: linear-gradient(hsl(var(--p)) 0.1em, transparent 0.1em),
    linear-gradient(90deg, hsl(var(--a)) 0.1em, transparent 0.1em);
